...
import { WindowsNotificationListener, DEFAULT_LOG_NAME } from './listeners/windowsNotificationListener.js';
import { WinRtNotificationListener } from './listeners/winRtNotificationListener.js';
...
const WINDOWS_DEBUG = process.env.AIS_WINDOWS_DEBUG === '1';

const wss = new WebSocketServer({ port: PORT });
let windowsListener = null;
let winRtListener = null;
...
async function setupWindowsListener() {
  ...
  windowsListener = new WindowsNotificationListener({
    ...
    onNotification: (notification) => {
      ...
      if (WINDOWS_DEBUG) {
        console.log('[AIS Bridge] Windows debug:', {
          source: notification.source,
          appName: notification.appName,
          sender: notification.sender,
          sourceRaw: notification.metadata?.sourceRaw ?? null,
          rawMessage: notification.metadata?.rawMessage ?? null,
        });
      }
  ...
  winRtListener = new WinRtNotificationListener({
    pollIntervalMs: WINDOWS_POLL_MS,
    onNotification: (notification) => {
      console.log(
        `[AIS Bridge] WinRT notification: source=${notification.source} id=${notification.id}`
      );
      if (WINDOWS_DEBUG) {
        console.log('[AIS Bridge] WinRT debug:', {
          source: notification.source,
          appName: notification.appName,
          sender: notification.sender,
    ...
    onError: (error) => {
      console.warn('[AIS Bridge] WinRT listener error:', error.message || error);
    },
  });

  try {
    await winRtListener.start();
    console.log(`[AIS Bridge] WinRT listener enabled (poll ${WINDOWS_POLL_MS}ms)`);
  } catch (error) {
    console.warn('[AIS Bridge] Failed to start WinRT listener:', error.message || error);
  }
}
